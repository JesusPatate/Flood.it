...
angular.module('floodit').factory('Bigint', function() {
  ...
  BigInt.millerRabinInt = function(x,b) {
    ...
    this.copyInt_(mr_a,b);
    return this.millerRabin(x,mr_a);
  },
  ...
  BigInt.millerRabin = function(x,b) {
    ...
    if (s)                
      this.rightShift_(mr_r,s);

    this.powMod_(mr_a,mr_r,x);

    if (!this.equalsInt(mr_a,1) && !this.equals(mr_a,mr_x1)) {
      j=1;
      while (j<=s-1 && !this.equals(mr_a,mr_x1)) {
        this.squareMod_(mr_a,x);
        if (this.equalsInt(mr_a,1)) {
          return 0;
        }
        j++;
      }
      if (!this.equals(mr_a,mr_x1)) {
        return 0;
      }
    }
    return 1;  
  ...
  BigInt.expand = function(x,n) {
    var ans=this.int2bigInt(0,(x.length>n ? x.length : n)*bpe,0);
    this.copy_(ans,x);
    return ans;
  },
  ...
  BigInt.randProbPrimeRounds = function(k,n) {
    ...
    if (primes.length==0)
      primes=this.findPrimes(30000);  //check for divisibility by primes <=30000
    ...
    for (;;) { //keep trying random values for ans until one appears to be prime
      ...
      for (i=0; (i<primes.length) && (primes[i]<=B); i++)
        if (this.modInt(ans,primes[i])==0 && !this.equalsInt(ans,primes[i])) {
          divisible=1;
          break;
        }      
      ...
      for (i=0; i<n && !divisible; i++) {
        this.randBigInt_(rpprb,k,0);
        while(!this.greater(ans,rpprb)) //pick a random rpprb that's < ans
          this.randBigInt_(rpprb,k,0);
        if (!this.millerRabin(ans,rpprb))
          divisible=1;
      }
  ...
  BigInt.randTruePrime_ = function(ans,k) {
    ...
    this.randTruePrime_(s_q,recSize);
    this.copyInt_(s_i2,0);
    s_i2[Math.floor((k-2)/bpe)] |= (1<<((k-2)%bpe));   //s_i2=2^(k-2)
    this.divide_(s_i2,s_q,s_i,s_rm);                        //s_i=floor((2^(k-1))/(2q))

    z=this.bitSize(s_i);

    for (;;) {
      for (;;) {  //generate z-bit numbers until one falls in the range [0,s_i-1]
        this.randBigInt_(s_R,z,0);
        if (this.greater(s_i,s_R))
          break;
      }                //now s_R is in the range [0,s_i-1]
      this.addInt_(s_R,1);  //now s_R is in the range [1,s_i]
      this.add_(s_R,s_i);   //now s_R is in the range [s_i+1,2*s_i]

      this.copy_(s_n,s_q);
      this.mult_(s_n,s_R); 
      this.multInt_(s_n,2);
      this.addInt_(s_n,1);    //s_n=2*s_R*s_q+1
      
      this.copy_(s_r2,s_R);
      this.multInt_(s_r2,2);  //s_r2=2*s_R

      //check s_n for divisibility by small primes up to B
      for (divisible=0,j=0; (j<primes.length) && (primes[j]<B); j++)
        if (this.modInt(s_n,primes[j])==0 && !this.equalsInt(s_n,primes[j])) {
          divisible=1;
          break;
        }      

      if (!divisible)    //if it passes small primes check, then try a single Miller-Rabin base 2
        if (!this.millerRabinInt(s_n,2)) //this line represents 75% of the total runtime for randTruePrime_ 
          divisible=1;
      ...
      if (!divisible) {  //if it passes that test, continue checking s_n
        ...
        for (;;) {  //generate z-bit numbers until one falls in the range [0,s_n-1]
          this.randBigInt_(s_a,zz,0);
          if (this.greater(s_n,s_a))
            break;
        }                //now s_a is in the range [0,s_n-1]
        this.addInt_(s_n,3);  //now s_a is in the range [0,s_n-4]
        this.addInt_(s_a,2);  //now s_a is in the range [2,s_n-2]
        this.copy_(s_b,s_a);
        this.copy_(s_n1,s_n);
        this.addInt_(s_n1,-1);
        this.powMod_(s_b,s_n1,s_n);   //s_b=s_a^(s_n-1) modulo s_n
        this.addInt_(s_b,-1);
        if (this.isZero(s_b)) {
          this.copy_(s_b,s_a);
          this.powMod_(s_b,s_r2,s_n);
          this.addInt_(s_b,-1);
          this.copy_(s_aa,s_n);
          this.copy_(s_d,s_b);
          this.GCD_(s_d,s_n);  //if s_b and s_n are relatively prime, then s_n is a prime
          if (this.equalsInt(s_d,1)) {
            this.copy_(ans,s_aa);
  ...
  BigInt.GCD_ = function(x,y) {
    ...
    while (sing) { //while y has nonzero elements other than y[0]
      ...
      if (B) {
        ...
        this.linComb_(y,T,D,C); //y=D*y+C*T
      } else {
        this.mod_(x,y);
        this.copy_(T,x);
        this.copy_(x,y);
  ...
  BigInt.inverseMod_ = function(x,n) {
    ...
    this.copy_(eg_u,x);
    this.copy_(eg_v,n);
    this.copyInt_(eg_A,1);
    this.copyInt_(eg_B,0);
    this.copyInt_(eg_C,0);
    this.copyInt_(eg_D,1);
    for (;;) {
      while(!(eg_u[0]&1)) {  //while eg_u is even
        ...
        if (!(eg_A[0]&1) && !(eg_B[0]&1)) { //if eg_A==eg_B==0 mod 2
          ...
          this.add_(eg_A,n);  this.halve_(eg_A);
          this.sub_(eg_B,x);  this.halve_(eg_B);
        }
      }
      ...
      while (!(eg_v[0]&1)) {  //while eg_v is even
        ...
        if (!(eg_C[0]&1) && !(eg_D[0]&1)) { //if eg_C==eg_D==0 mod 2
          ...
          this.add_(eg_C,n);  this.halve_(eg_C);
          this.sub_(eg_D,x);  this.halve_(eg_D);
        }
      }

      if (!this.greater(eg_v,eg_u)) { //eg_v <= eg_u
        this.sub_(eg_u,eg_v);
        this.sub_(eg_A,eg_C);
        this.sub_(eg_B,eg_D);
      } else {                   //eg_v > eg_u
      ...
      if (this.equalsInt(eg_u,0)) {
        while (this.negative(eg_C)) //make sure answer is nonnegative
          this.add_(eg_C,n);
        this.copy_(x,eg_C);

        if (!this.equalsInt(eg_v,1)) { //if GCD_(x,n)!=1, then there is no inverse
          this.copyInt_(x,0);
          return 0;
        }
        return 1;
  ...
  BigInt.inverseModInt_ = function(x,n) {
  ...
  BigInt.eGCD_ = function(x,y,v,a,b) {
    var g=0;
    ...
    for (;;) {
      while(!(eg_u[0]&1)) {  //while u is even
        ...
        if (!(eg_A[0]&1) && !(eg_B[0]&1)) { //if A==B==0 mod 2
          ...
          this.add_(eg_A,y);  this.halve_(eg_A);
          this.sub_(eg_B,x);  this.halve_(eg_B);
        }
      }
      ...
      while (!(v[0]&1)) {  //while v is even
        ...
        if (!(eg_C[0]&1) && !(eg_D[0]&1)) { //if C==D==0 mod 2
          ...
          this.add_(eg_C,y);  this.halve_(eg_C);
          this.sub_(eg_D,x);  this.halve_(eg_D);
        }
      }

      if (!this.greater(v,eg_u)) { //v<=u
        this.sub_(eg_u,v);
        this.sub_(eg_A,eg_C);
        this.sub_(eg_B,eg_D);
      } else {                //v>u
      ...
      if (this.equalsInt(eg_u,0)) {
        while (this.negative(eg_C)) {   //make sure a (C) is nonnegative
          this.add_(eg_C,y);
          this.sub_(eg_D,x);
        }
        this.multInt_(eg_D,-1);  ///make sure b (D) is nonnegative
        this.copy_(a,eg_C);
        this.copy_(b,eg_D);
        this.leftShift_(v,g);
        return;
  ...
  BigInt.bigInt2str = function(x,base) {
    ...
    if (base==-1) { //return the list of array contents
      ...
      while (!this.isZero(s6)) {
        t=this.divInt_(s6,base);  //t=s6 % base; s6=floor(s6/base);
  ...
  BigInt.mult_ = function(x,y) {
    ...
    for (i=0;i<y.length;i++)
      if (y[i])
        this.linCombShift_(ss,x,y[i],i);   //ss=1*ss+y[i]*(x<<(i*bpe))
    this.copy_(x,ss);
  },
  ...
  BigInt.mod_ = function(x,n) {
    ...
    if (s5.length!=x.length)
      s5=this.dup(x);  
    this.divide_(s4,n,s5,x);  //x = remainder of s4 / n
  },
  ...
  BigInt.multMod_ = function(x,y,n) {
    ...
    for (i=0;i<y.length;i++)
      if (y[i])
        this.linCombShift_(s0,x,y[i],i);   //s0=1*s0+y[i]*(x<<(i*bpe))
    this.mod_(s0,n);
    this.copy_(x,s0);
  },
  ...
  BigInt.powMod_ = function(x,y,n) {
    ...
    if ((n[0]&1)==0) {
      ...
      while(!this.equalsInt(y,0)) {
        if (y[0]&1)
          this.multMod_(x,s7,n);
        this.divInt_(y,2);
        this.squareMod_(s7,n); 
      }
      return;
    }
    ...
    np=radix-this.inverseModInt(this.modInt(n,radix),radix);
    s7[kn]=1;
    this.multMod_(x ,s7,n);   // x = x * 2**(kn*bp) mod n
    ...
    for (;;) {
      if (!(k2>>=1)) {  //look at next bit of y
        k1--;
        if (k1<0) {
          this.mont_(x,this.int2bigInt(1,1,1),n,np);
          return;
        }
        k2=1<<(bpe-1);
      }    
